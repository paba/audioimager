...
public class FlickrPhoto {
     ...
     * @throws java.io.IOException
     */
    private void getSizes() throws MalformedURLException, SAXException, IOException, FlickrException {
        ...
        Document flickrData = flickrClient.getFlickrData(METHOD_PHOTO_SIZES, flickrApiParams);

        // Get the list of tags "sizes" corresponding to each Flickr photo
        NodeList sizes = flickrData.getDocumentElement().getElementsByTagName("sizes");
        if (sizes != null && sizes.getLength() > 0) {
            // Get the list of tags "sizes" corresponding to each Flickr photo
            NodeList sizeList = ((Element) sizes.item(0)).getElementsByTagName("size");
            if (sizeList != null && sizeList.getLength() > 0) {
                for (int i = 0; i < sizeList.getLength(); i++) {
                    Element photoSizeInfo = (Element) sizeList.item(i);
                    imageSizeMap.put(photoSizeInfo.getAttribute("label"), photoSizeInfo);
