...
/**
 * Creates a Thread to load images from a given flickr account.
...
public class MyFlickrImageLoadThread extends Thread {
	...
	public MyFlickrImageLoadThread(ArrayList<MyFlickrImage> myFlickrImageList,
			PhotoList photoList, JTable tblMyFlickrImages,
			TableModel tblMyFlickrImagesModel, JLabel lblWait,
			JLabel lblStatus, int myFlickrMoreCount) {
		this.myFlickrImageList = myFlickrImageList;
		this.photoList = photoList;
		this.tblMyFlickrImages = tblMyFlickrImages;
		resizeImage = new ResizeImage(90, 120, "CENTER");
	...
	public void run() {
		...
		for (int i = 0; i < photoList.size(); i++) {
			...
			try {
				...
				if (resizedImage != null) {
					MyFlickrImage myFlickrImage = new MyFlickrImage(new URL(
							photo.getMediumUrl()), resizedImage);
		...
		for (int i = position; i < (count / 4) * 4; i++) {
			...
			tblMyFlickrImages.getColumnModel().getColumn(i % 4)
					.setCellRenderer(flickrCellRenderer);
			tblMyFlickrImagesModel.setValueAt(resizedImage, (int) Math
