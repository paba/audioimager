...
/**
 * Creates an image with attributions.
...
public class CCAttribution {
	...
	public CCAttribution(ArrayList<AttributionData> attributionList,
			Component comp) {
		this.attList = attributionList;
		this.component = comp;
		resize = new ResizeImage(240, 320, "CENTER");
	...
	public Image createAttributionImage() {
		...
		for (int i = 0; i < attList.size(); i++) {
			...
			try {
				s = "-"
						+ URLDecoder.decode(attList.get(i).getTitle(),
								"UNICODE");
				bimg.getGraphics().drawString(s, 2, y);
				bimg.getGraphics().drawString(
						"by "
								+ URLDecoder.decode(attList.get(i)
										.getOwnerName(), "UNICODE"), 4, y + 15);
				if (attList.get(i).getUrl() != null) {
					bimg.getGraphics().drawString(
							attList.get(i).getUrl().toString(), 4, y + 30);
				}
		...
		bimg.getGraphics().drawString("~~Created by Using AudioImager~~", 60,
				235);
